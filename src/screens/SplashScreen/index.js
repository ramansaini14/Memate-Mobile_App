...
const SplashScreen = ({navigation}) => {
  ...
  useEffect (() => {
    console.log("Required ===> ",isRequired, "isPinCreate ===> ",isPinCreate," isAppTerm ===> ",isAppTerm)
    if(loading){
    if (!token) navigation.navigate('StartScreen');
    else if (isNew && !isPinCreate) navigation.navigate('CreatePin');
    else if (isNew && !isAppTerm) navigation.navigate('TermsAndConditions', {from: 'pin', id: 0});
    else if (isNew && isRequired) navigation.navigate('RequireDetails');
